## Scope
Fail2ban is dope and SSH is quite secure, but what if someone still manages to authenticate to your machine e.g. by using saved/harvested credentials?
Recieve email alerts on successful ssh logins based on a predefined IP whitelist OR a predefined IP country origin whitelist.
Essentially: IF (ssh succesful authentication ip address NOT IN ip whitelist) OR (ssh succesful authentication ip address country of origin NOT IN country whitelist); then send email notification; 
...
`./ssh-log-alert.sh`
2. Add script to the root crontab and have it run in the background the machine starts:
`crontab -e`
then add line:
